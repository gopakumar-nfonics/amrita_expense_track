							...
							<div class="card-body p-0">
								...
								<div class="px-9 pt-7 card-rounded h-275px w-100 bg-blue">
									...
									<div class="d-flex text-center flex-column text-white pt-8">
										...
										<span class="fw-bold fs-2x pt-1">&#x20b9;{{ number_format($budgettotalAmount, 2) }}</span>
									</div>
									<!--end::Balance-->
								</div>
								<!--end::Header-->
								...
								<div class="bg-body shadow-sm card-rounded mx-9 mb-9 px-6 py-9 pb-5 position-relative z-index-1" style="margin-top: -100px">
									...
									@php
									$allocatedPercentage = $budgettotalAmount > 0 ? ($budget->total_amount / $budgettotalAmount) * 100 : 0;
									if (floor($allocatedPercentage) == $allocatedPercentage) {
									$allocatedPercentage = number_format($allocatedPercentage, 0) . '%'; // Use . to concatenate
									} else {
									$allocatedPercentage = number_format($allocatedPercentage, 2) . '%'; // Use . to concatenate
									}
									...
									$output = strtoupper(substr($words[0], 0, 1) . substr($words[count($words) - 1], 0, 1));
									}
									...
									<div class="d-flex align-items-center mb-6">
										...
										<div class="d-flex align-items-center flex-wrap w-100">
											...
											<div class="mb-1 pe-3 flex-grow-1">
												<a href="#" class="fs-5 text-gray-800 text-hover-primary fw-bold">{{$budget->category->category_name }}</a>
												<div class="text-gray-400 fw-semibold fs-7">
													<span class="badge badge-light-success fs-base">
														...
														<!--end::Svg Icon-->{{$allocatedPercentage }}</span>
												</div>
											</div>
											<!--end::Title-->
											<!--begin::Label-->
											<div class="d-flex align-items-center">
												<div class="fw-bold fs-5 text-gray-800 pe-1">&#x20b9;{{ number_format($budget->total_amount, 2) }}</div>
							...
							<div class="card-body p-0">
								...
								<div class="px-9 pt-7 card-rounded h-275px w-100 bg-danger">
									...
									<div class="d-flex text-center flex-column text-white pt-8">
										...
										<span class="fw-bold fs-2x pt-1">&#x20b9;{{ number_format($totalPaidAmount, 2) }}</span>
									</div>
									<!--end::Balance-->
								</div>
								<!--end::Header-->
								...
								<div class="bg-body shadow-sm card-rounded mx-9 mb-9 px-6 py-9 pb-5 position-relative z-index-1" style="margin-top: -100px">
									...
									$boutput = strtoupper(substr($bwords[0], 0, 1) . substr($bwords[count($bwords) - 1], 0, 1));
									}
									@endphp
									<div class="d-flex align-items-center mb-6">
										<!--begin::Symbol-->
										...
										<div class="d-flex align-items-center flex-wrap w-100">
											<!--begin::Title-->
											<div class="mb-1 pe-3 flex-grow-1">
												<a href="#" class="fs-5 text-gray-800 text-hover-primary fw-bold">{{ $result['parent_category_name'] }}</a>
												<div class="text-gray-400 fw-semibold fs-7">
													<div class="d-flex flex-column w-100 me-2">
														<span class="text-muted me-2 fs-8 fw-bold">{{ number_format($catpaidPercentage, 2) }}%</span>

														<div class="progress h-4px w-150px p-0 m-0">

															<div class="progress-bar {{$catprogressBarClass}}" role="progressbar" style="width: {{ floor($catpaidPercentage) }}%" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100"></div>
														</div>
													</div>
												</div>
											</div>
											...
											<!--begin::Label-->
											<div class="d-flex align-items-center">
												<div class="fw-bold fs-5 text-gray-800 pe-1">&#x20b9;{{ number_format($result['total_milestone_amount'], 2) }}</div>
							...
							<div class="card-body p-0">
								...
								<div class="px-9 pt-7 card-rounded h-275px w-100 bg-success">
									...
									<div class="d-flex text-center flex-column text-white pt-8">
										...
										<span class="fw-bold fs-2x pt-1">&#x20b9;{{ number_format($remainingBudget, 2) }}</span>
									</div>
									<!--end::Balance-->
								</div>
								<!--end::Header-->
								...
								<div class="bg-body shadow-sm card-rounded mx-9 mb-9 px-6 py-9 position-relative z-index-1" style="margin-top: -100px">
									<div class="card-body d-flex flex-column  py-0">
										<div class="flex-grow-1">
											<div class="budgetused" data-kt-chart-color="success" style="height: 200px; min-height: 178.7px;" id="usedPercentageChart">
					...
					<div class="card mb-5">
						...
						<div class="card-body py-3">
							...
							<div class="table-responsive">
								<!--begin::Table-->
								<table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4" id="vendor-table">
									<!--begin::Table head-->
									<thead>
									...
									<tbody>
										...
										$paidPercentage = $totalProposalAmount > 0 ? ($totalPaidAmount / $totalProposalAmount) * 100 : 0;
									...
									$voutput = strtoupper(substr($vwords[0], 0, 1) . substr($vwords[count($vwords) - 1], 0, 1));
									}
										@endphp
										<tr>
											...
											<td>
												<div class="d-flex align-items-center">
													<div class="symbol symbol-45px me-5">
														<span class="symbol-label bg-blue text-white"> {{$voutput}}</span>

													</div>
													<div class="d-flex justify-content-start flex-column">
														<a href="{{ route('vendor.show',1) }}" class="text-dark fw-bold text-hover-primary fs-6">{{strtoupper($vendor->vendor_name)}}</a>
														<span class="text-muted fw-semibold text-muted d-block fs-7">{{$vendor->email}}</span>
													</div>
												</div>
											</td>
											<td>
												<a href="#" class="text-dark fw-bold text-hover-primary d-block fs-6">{{ $vendor->company->company_name ?? '' }}</a>
												<!--<span class="text-muted fw-semibold text-muted d-block fs-7">+91-898-589-0432</span>-->
											</td>
											<td>
												₹{{ number_format($totalProposalAmount, 2) }}
											</td>
											<td>
											...
											<td class="text-end">
												<div class="d-flex flex-column w-100 me-2">
													<div class="d-flex flex-stack mb-2">
														<span class="text-muted me-2 fs-7 fw-bold">{{ number_format($paidPercentage, 2) }}%</span>
													</div>
													<div class="progress h-6px w-100">
														<div class="progress-bar {{ $progressBarClass }}" role="progressbar" style="width: {{ floor($paidPercentage) }}%;" aria-valuenow="90" aria-valuemin="0" aria-valuemax="100"></div>
													</div>
												</div>
											</td>
...
<script>
	...
	var initMixedWidget4 = function() {
		...
		[].slice.call(charts).map(function(element) {
			...
			var lightColor = getComputedStyle(document.documentElement).getPropertyValue('--kt-' + color + '-light');
