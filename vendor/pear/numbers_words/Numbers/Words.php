...
class Numbers_Words
{
     ...
     */
     ...
     function toWords($num, $locale = '', $options = array())
     {
         ...
         if (empty($locale)) {
             $locale = 'en_IN'; // Change to Indian locale
         }
         ...
         // Custom logic for Indian numbering system
         $num = (int)$num;
         $result = '';
         ...
         if ($crores > 0) {
             $result .= $obj->_toWords($crores) . ' crore ';
         }
         if ($lakhs > 0) {
             $result .= $obj->_toWords($lakhs) . ' lakh ';
         }
         if ($thousands > 0) {
             $result .= $obj->_toWords($thousands) . ' thousand ';
         }
         if ($hundreds > 0) {
             $result .= $obj->_toWords($hundreds) . ' hundred ';
         }
         if ($remainder > 0) {
             $result .= $obj->_toWords($remainder);
         }
