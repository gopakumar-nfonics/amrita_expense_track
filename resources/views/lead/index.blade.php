                                                        ...
                                                        <div class="text-gray-400 fw-semibold fs-9">
                                                            ...
                                                            @elseif($pro->proposal_status == 2)
                                                                ...
                                                                <span class="badge badge-light-info fs-8 rejected-span"
                                                                    ...
                                                                    <!--end::Svg Icon-->
                                                                    <i
                                                                        class="fa-regular fa-comments color-blue fs-8 me-2 "></i>
                                                    ...
                                                    @if (!Auth::user()->isvendor() && $pro->proposal_status == 0)
                                                        <div class="menu-item px-3">
                                                            <a href="javascript:void(0)"
                                                                onclick="approve('{{ $pro->id }}','approve')"
...
@section('pageScripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            var yearColIndex = $('#budgettable thead th').filter(function() {
                return $(this).text().trim() === 'Proposal Year';
            }).index();

            $('#financialYearSelect')
                .appendTo('#budgettable_wrapper .dataTables_filter')
                .css({
                    'margin-left': '20px',
                    'width': '100px',
                    'height': '45px'
                ...
                .on('change', function() {
                    ...
                    table.column(yearColIndex).search(selectedYear ? selectedYear : '', true, false).draw();
                });

            var initialYear = $('#financialYearSelect').val();
            if (initialYear) {
                table.column(yearColIndex).search(initialYear, true, false).draw();
            }

            $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                if (settings.nTable.id !== 'budgettable') {
                    return true;
                }
                var yearCellHtml = data[yearColIndex] || '';
                var yearCellText = $('<div>').html(yearCellHtml).text().trim();

                var selectedYear = $('#financialYearSelect').val();
                if (!selectedYear) return true;
                return yearCellText === selectedYear;
            });
