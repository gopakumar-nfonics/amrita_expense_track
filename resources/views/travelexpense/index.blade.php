                                    ...
                                    @forelse($expenses as $key => $expense)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <div
                                                        class="d-flex justify-content-start flex-column text-dark fw-bold fs-5">
                                                        {{ $expense->staff->name }}
                                                        <span class="text-muted fw-semibold text-muted d-block fs-8">
                                                            <i class="fa-regular fa-envelope fs-8 me-1"></i>
                                                            {{ $expense->staff->email }}
                                                        </span>
                                                        <span class="text-muted fw-semibold text-muted d-block fs-8">
                                                            <i class="fa-regular fa-user fs-8 me-1"></i>
                                                            {{ $expense->staff->designation->title }}
                                                        </span>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-150px py-4 ps-3"
                                                    data-kt-menu="true">
                                                    ...
                                                    @if (in_array($expense->status, ['expense_submitted', 'expense_settled']))
                                                        <div class="menu-item px-1">
                                                            <a href="{{ route('travelexpense.show', encrypt($expense->id)) }}"
                                                                ...
                                                                {{ $expense->status === 'expense_submitted' ? 'View & Approve' : 'View' }}
                                                            </a>
                                                        </div>
                                                    @endif
                                                    @if ($expense->status === 'advance_requested')
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <select name="associated" class="form-select" required>
                                                        <option value="">-- Select --</option>
                                                        <option value="Vendor Associate">
                                                            Vendor Associate
                                                        </option>
                                                        <option value="DOA">
                                                            DOA
        ...
        function removeExpense(expenseId) {
            swal({
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            url: "{{ route('travel.deleterequest') }}",
                            type: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                id: expenseId,
